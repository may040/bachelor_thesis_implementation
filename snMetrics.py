...
for e in epsilons:
    ...
    readerD1 = snsql.from_df(ageListD1, privacy=privacy, metadata=meta_path)

    d2_csv_path = r'd2Data.csv'
    meta_path = r'age.yaml'
    ageListD2 = pd.read_csv(d2_csv_path)
    privacy = Privacy(e, delta=delta)
    readerD2 = snsql.from_df(ageListD2, privacy=privacy, metadata=meta_path)
...
file_exists_result = os.path.isfile('snResults.csv')
with open('snResults.csv', 'a+', newline='') as csvfile:
    fieldnames = ['epsilon'] + members
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    if not file_exists_result:
        writer.writeheader()
    for i in range(0, 5):
        values = [getattr(results[i], member) for member in members]
